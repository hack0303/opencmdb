...
export async function getAssets(filters: AssetFilters): Promise<AssetsResponse> {
  ...
  // Default: bound assets first (order 0), then unbound (order 1), both by recency
  let orderBy = `ORDER BY CASE WHEN a.id IN (SELECT asset_id FROM service_asset_bindings WHERE deleted_at IS NULL) THEN 0 ELSE 1 END, a.updated_at DESC`;
  if (sort) {
    try {
      const sortItems = JSON.parse(sort) as { id: string; desc: boolean }[];
      if (sortItems.length > 0) {
